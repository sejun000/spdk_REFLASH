...
#define FTL_FDP_METADATA_ENABLED	1
 ...
 * NOTE: iov must be caller-provided and persist until I/O completes (not stack variable!)
 */
static inline int
					   ...
					   struct iovec *iov)
{
	...
	/* Setup iov - caller must ensure iov persists until I/O completes */
	iov->iov_base = buf;
	iov->iov_len = num_blocks * FTL_BLOCK_SIZE;
	...
	if (spdk_bdev_get_md_size(spdk_bdev_desc_get_bdev(desc))) {
		opts.metadata = md ? : g_ftl_write_buf;
	}
	...
	(void)iov;  /* unused when FDP disabled */

...
CACHE_SPLIT_GB=${CACHE_SPLIT_GB:-20}
...
if ! "${RPC[@]}" bdev_get_bdevs -b "$CACHE_DEVICE" >/dev/null 2>&1; then
	log "Limiting cache namespace ${CACHE_NS} to ${CACHE_SPLIT_GB}GB"
	rpc_call "split cache namespace ${CACHE_NS}" \
		bdev_split_create --split-size-mb "$CACHE_SPLIT_MB" "$CACHE_NS" 1
	...
	if ! "${RPC[@]}" bdev_get_bdevs -b "$CACHE_DEVICE" >/dev/null 2>&1; then
		echo "[create_tier] Failed to create split ${CACHE_DEVICE}" >&2
		exit 1
	fi
fi
...
if [[ "${NVMF_ENABLE}" != "0" ]]; then
	...
	sleep 2
	rpc_call "create subsystem ${NVMF_SUBSYSTEM}" \
		nvmf_create_subsystem "${NVMF_SUBSYSTEM}" -a -s "${NVMF_SERIAL}"
	sleep 2
	rpc_call "add namespace ${ICACHE_NAME} to ${NVMF_SUBSYSTEM}" \
		nvmf_subsystem_add_ns "${NVMF_SUBSYSTEM}" "${ICACHE_NAME}"
	sleep 2
	rpc_call "add listener ${NVMF_SUBSYSTEM}@${NVMF_TRADDR}:${NVMF_TRSVCID}" \
		nvmf_subsystem_add_listener "${NVMF_SUBSYSTEM}" \
		-t "${NVMF_TRTYPE}" -f "${NVMF_ADRFAM}" -a "${NVMF_TRADDR}" -s "${NVMF_TRSVCID}"
fi
